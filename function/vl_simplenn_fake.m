function res = vl_simplenn_fake(net, x, dzdy, res, varargin)
...
for i=1:n
  ...
  switch l.type
    ...
    case 'sigmoidcrossentropyloss'
      res(i+1).x = sigmoidcrossentropyloss(res(i).x, l.class);
      %fprintf('\nmax_output_layer,%d\n',max(max(max(max(res(i).x)))));

    %  fprintf('\nmean_output_layer,%d\n',mean(mean(mean(mean(res(i).x)))));
     % fprintf('mean_labels_class,%d\n',mean(mean(mean(mean(l.class)))));
    case 'euclideanloss'
