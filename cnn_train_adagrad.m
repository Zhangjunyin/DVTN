...
switch opts.errorType
  ...
  case 'euclideanloss'
    ...
    %fprintf('\n\nm_sigmoid(predict) : %d\n',mean(mean(mean(mean(sigmoid(predictions))))));
    %fprintf('m_label : %d\n\n',mean(mean(mean(mean(labels)))));
    %output1 = (sigmoid(predictions(:,:,:,1)));
    %output2 = uint8(output1);
    %output =uint8(255.*ones(30,30)) - uint8((output - min(min(output)))/(max(max(output)) - min(min(output))) .* 255);
    %  imwrite(labels(:,:,1,1),'~/dataset/labels_1_1.png');
    %  imwrite(output2,'~/dataset/predict.png');
      %labels(:,:,1,1)
      %output(:,:,:,1)
end
